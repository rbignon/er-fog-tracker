...
def generate_report_md(
    ...
    lines = [
        ...
        "",
        "- `fog.txt` - Zone definitions, display names, aliases, and fog gate data",
        "- `submaps.txt` - Position-based zone rules",
        "- `foglocations2.txt` - Zone location data by map (Col-based lookup)",
        "- `graces.json` - Graces mapping for zone_query",
        "",
        "### Documentation (`docs/`)",
        "",
        "- `ZONE_MATCHING.md` - Zone resolution strategies, data file formats, "
        "troubleshooting guide",
        "- `GRAPH_MODEL.md` - Zone links, discovery logic, one-way connections",
        "",
        ...
        "If the fix involves:",
        "",
        "- New resolution strategies or edge cases → update `docs/ZONE_MATCHING.md`",
        "- Changes to data file formats → update both `docs/ZONE_MATCHING.md` "
        "and relevant code docstrings",
        "- New graph/discovery logic → update `docs/GRAPH_MODEL.md`",
        "",
    ]
